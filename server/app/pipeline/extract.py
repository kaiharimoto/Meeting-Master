...
# The live path asks for the SAME Q&A pairs, from a partial and rough
# transcript. Separate from SYSTEM_PROMPT because the instructions differ in
# kind, not just in degree: silence beats speculation here (the operator is in
# a meeting), and the post-meeting run over the full transcript is what catches
# anything held back.
#
# It used to ask for "insights" too — candidate Key Insights the operator kept
# into the summary. That half was retired in v0.22.0 and replaced by the
# meeting progress map (run_map below), which shows where the meeting has got
# to rather than offering one-line lessons to approve. summary.keyInsights is
# untouched: it still comes from the post-meeting pass over the full
# transcript.
...
LIVE_SYSTEM_PROMPT = (
    ...
    "speculative one. Return questions that were asked AND already answered in this "
    ...
    "otherwise 'low'.\n\n"
    "Most excerpts of a live meeting contain no fully answered question; an "
    "empty array is the correct and expected answer then.\n\n"
    "Use ONLY what is in the excerpt. Never invent questions, answers or "
    "names. The array may be empty. " + _LIVE_SCHEMA_HINT
)
...
async def run_live(
    ...
    """Suggest Q&A pairs from a PARTIAL live transcript window.
    ...
    ``already_flagged`` is what the operator has already been offered this
    session; it is excluded from the prompt AND filtered from the result, so
    the rail never shows the same question twice.
    """
